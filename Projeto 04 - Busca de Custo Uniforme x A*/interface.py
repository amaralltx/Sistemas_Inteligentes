...
class Interface:
    ...
    def __init__(self, janela, ambiente, agente):
        ...
        self.frame_labirinto = CTkFrame(
            self.janela,
            width=750,
            height=750,
            bg_color=self.branco,
            fg_color=self.branco,
            corner_radius=0,
        )
        self.frame_input_inicial = CTkFrame(
            self.janela, width=500, height=750, fg_color=self.preto
        )
    ...
    def inicializa_frame_input_inicial(self):
        ...
        """
        self.frame_input_inicial_baixo = CTkFrame(
            self.frame_input_inicial, width=300, height=100, fg_color="transparent"
        )
        self.frame_input_inicial_cima = CTkFrame(
            self.frame_input_inicial, width=450, height=100, fg_color="transparent"
        )
    ...
    def configurar_bind_enter(self):
        """
        Configura o bind da tecla Enter para os widgets de entrada enviarem o formulário.
        """
    ...
    def inicializa_frame_input_principal(self):
        ...
        self.input_comando.grid(
            row=1, column=0, padx=(15, 0), pady=15, sticky="nsew", ipadx=10, ipady=10
        )
        self.input_comando.bind("<Return>", lambda event: self.receber_comando())
    ...
    def mostrar_labirinto(self):
        ...
        self.imagem_saida = self.imagem_saida.resize(
        ...
        for i in range(altura):
            for j in range(largura):
                ...
                if estrutura_labirinto[i][j] == "⬛":  # '⬛' representa uma parede
                    ...
                    self.imagem_saida = self.canvas_labirinto.create_image(
                        x0, y0, anchor="nw", image=self.imagem_saida_tk
                    )
                elif (
                    estrutura_labirinto[i][j] == "🟦"
                ):  # '🟦' representa o objeto_movel explorador
                    cor = self.branco
                    self.canvas_labirinto.create_rectangle(
                        x0, y0, x1, y1, outline="#C1C1C1", fill=cor
                    )
    ...
    def validar_mover_objeto_movel(self, direcao):
        ...
        if direcao is None:
            ...
            return False
        validador, mensagem_de_erro, deslocamento = (
            self.ambiente.validar_movimento_objeto_movel(direcao)
        )
    ...
    def exibir_mensagem(self, mensagem):
        """
        Exibe uma mensagem no console do programa (caixa_texto).
        """
...
def dividir_string(s):
    """
    Divide a string em duas strings: comando e argumentos.
    """
